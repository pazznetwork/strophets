...
export class BoshRequest {
  ...
  process(i: number, connection: Connection, primaryTimeout: number): void {
    if (this.xhr.readyState === 0) {
      debug('request id ' + this.rid + '.' + this.sends + ' posting');

      try {
        const content_type = connection.options.contentType || 'text/xml; charset=utf-8';
        this.xhr.open('POST', connection.service, !connection.options.sync);
        this.xhr.setRequestHeader('Content-Type', content_type);
        if (connection.options.withCredentials) {
      ...
      const sendFunc = (): void => {
        ...
        if (connection.options.customHeaders) {
          const headers = connection.options.customHeaders;
