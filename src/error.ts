...
type StropheError = Error & StropheErrorFields;
...
export function handleError(e: StropheError, handler?: Handler): void {
  ...
  if (e.sourceURL) {
    fatal('error: ' + handler + ' ' + e.sourceURL + ':' + e.line + ' - ' + e.name + ': ' + e.message);
  } else if (e.fileName) {
    fatal('error: ' + handler + ' ' + e.fileName + ':' + e.lineNumber + ' - ' + e.name + ': ' + e.message);
  } else {
